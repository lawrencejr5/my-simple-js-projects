...
form.addEventListener("submit", function(e){
    ...
    if(isNaN(value) || value <= 0 || value > 9){
        paragraphContainer.innerHTML =  `<div class="paragraph"> ${paragraphs[randomNumber]}</div>`
    }else{
        let newParagraphs = paragraphs.slice(0, value)
        let allParagrapghs = newParagraphs.map(function(i){
            return `<div class="paragraph"> ${i}</div>`
        }).join("")
        paragraphContainer.innerHTML =  allParagrapghs
